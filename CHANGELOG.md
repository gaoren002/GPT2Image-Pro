...
不可用时继续完成提示词修复，并缩短 Web 账号异常后的切换时间。

### 修复

- **审核提示词修剪增加 Web 回退**：审核拦截后仍优先使用 Codex/Responses 或外接 `/responses`；初始无可用 Responses 后端或运行时重试耗尽时，跨组选择 ChatGPT Web 账号，以 GPT-5.5 低思考纯文本会话完成修剪。Web 纯文字请求不受图片额度限制，不会清除或误减图片侧的额度与冷却状态；轮询遇到鉴权、限流或服务错误时会立即换号。

### 升级说明

- 本版本不包含数据库迁移。
- 提示词修剪仍优先使用 Codex/Responses；仅在没有可用账号或运行时重试耗尽时回退到 ChatGPT Web。
